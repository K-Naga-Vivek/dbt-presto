...
|---------|----------------------------------------------------|-------------------------|--------------------------|
| method  | The Presto authentication method to use | Optional (default is `none`)  | `none` or `BasicAuth` |
| user  | Username for authentication | Required  | `drew` |
| password  | Password for authentication | Optional (required if `method` is `BasicAuth`)  | `none` or `abc123` |
...
| http_scheme | The HTTP scheme to use for requests to Presto | Optional (default is `http`, or `https` for `method: BasicAuth`) | `https` or `http`
